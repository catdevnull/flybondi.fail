<script lang="ts">
	...
	function updateCharts() {
		...
		const datasets = airlines.map((code) => {
			const airlineData = data.monthlyStats
		...
		const config: ChartConfiguration<'line'> = {
			...
			options: {
				...
				plugins: {
					title: {
						display: true,
						text: 'Promedio mensual de retraso en despegue (minutos)',
						font: {
							size: 16,
							weight: 'bold'
						},
					...
					tooltip: {
						callbacks: {
							...
							title: (context: TooltipItem<'line'>[]) => {
								return dayjs(context[0].parsed.x).format('MM/YYYY');
							}
						}
					}
				},
				scales: {
					x: {
						...
						min: new Date(startDate.year, startDate.month - 1, 1).getTime(),
						max: new Date(endDate.year, endDate.month - 1, 1).getTime(),
						ticks: {
							maxRotation: 0,
							autoSkip: true,
							font: {
				...
				onClick: (event: ChartEvent, elements: any[]) => {
					if (elements.length > 0) {
						...
						if (airline) {
							// Navegar al histórico diario del mes clickeado
							const month = dayjs(dataPoint.x).format('YYYY-MM');
							const startOfMonth = dayjs(month + '-01').format('YYYY-MM-DD');
							const endOfMonth = dayjs(month + '-01')
								.endOf('month')
								.format('YYYY-MM-DD');
							goto(`/historico?start=${startOfMonth}&end=${endOfMonth}&aerolinea=${airline}`);
						}
					}
				}
			}
		...
		const cancelConfig: ChartConfiguration<'line'> = {
			...
			options: {
				...
				plugins: {
					title: {
						display: true,
						text: 'Porcentaje mensual de vuelos cancelados',
						font: {
							size: 16,
							weight: 'bold'
						},
	...
	$: metaTitle = `Estadísticas históricas mensuales - failbondi.fail`;
	$: metaDescription = `Estadísticas históricas mensuales de retrasos y cancelaciones de vuelos en Argentina del ${formatDate(startDate.toDate(tsz))} al ${formatDate(endDate.toDate(tsz))}`;
	...
	$: flybondiStats = data
		? {
				filtered: data.monthlyStats.filter((d) => d.airline === 'FO') as MonthlyStat[],
				totalFlights: data.monthlyStats
					.filter((d) => d.airline === 'FO')
					.reduce((sum, d) => sum + safeParseInt((d as any).total_flights), 0)
			}
		: { filtered: [] as MonthlyStat[], totalFlights: 0 };
...
<svelte:head>
	...
	<meta property="og:url" content="https://failbondi.fail/historico-meses" />
	...
	<meta name="twitter:url" content="https://failbondi.fail/historico-meses" />
...
<div class="prose prose-neutral dark:prose-invert mx-auto mt-8 max-w-[600px] p-4 sm:mt-12">
	...
	<ul>
		...
		<li>
			Se excluyen las aerolíneas que tienen menos de 50 vuelos por mes para evitar datos
			estadísticamente poco significativos.
		</li>
		<li>
			El retraso promedio mensual se calcula como la diferencia promedio en minutos entre la hora
			programada y la hora real de despegue durante el mes.
		</li>
		<li>
		...
		<li>
			Los meses sin datos o con menos de 50 vuelos por aerolínea no son seleccionables en el
			calendario.
		</li>
		<li>Hacer clic en un punto del gráfico navega al histórico diario de ese mes.</li>
	</ul>
